...
class Store {
  ...
  sell(marketId: string, side: Side, price: number, quantity: number, outcomeId?: string) {
    if (!this.user) throw new Error("User not authenticated");

    // Find position specific to this USER
    const posIdx = this.positions.findIndex(p => p.userId === this.user!.id && p.marketId === marketId && p.side === side && p.outcomeId === outcomeId);
    ...
    // Calculate commission on the sale
    const market = this.getMarket(marketId);
    const commissionPercent = market?.commission || 0;
    const tradingFee = this.config.value.tradingFee || 0;
    const commissionAmount = Math.round((revenue * commissionPercent) / 100);
    const netRevenue = revenue - commissionAmount - tradingFee; // Net revenue after commission and fee

    // PnL based on Net Revenue (after fees) vs Cost Basis (which includes buy fees)
    const profitOrLoss = netRevenue - costBasis;

    // Update user balance with net revenue
    this.updateUser(this.user.id, (u) => ({
      balance: u.balance + netRevenue,
      withdrawableBalance: u.withdrawableBalance + netRevenue
    }));

    this.performance.totalRealizedPnL += profitOrLoss;
    this.performance.bestTrade = Math.max(this.performance.bestTrade, profitOrLoss);
    this.performance.allTimeVolume += revenue;
    ...
    if (commissionAmount > 0) {
      this.ledger = [{
        ...
        currency: 'NPR',
        type: LedgerType.TRADE_FEE,
        description: `Commission for selling ${quantity} shares of ${market?.title || 'Unknown Market'}`,
        createdAt: new Date().toISOString(),
        status: 'COMPLETED'
      }, ...this.ledger];
    }
    ...
    if (tradingFee > 0) {
      this.ledger = [{
        ...
        currency: 'NPR',
        type: LedgerType.TRADE_FEE,
        description: `Fixed Trading Fee for selling ${quantity} shares`,
        createdAt: new Date().toISOString(),
        status: 'COMPLETED'
      }, ...this.ledger];
    }

    const ledgerEntry: LedgerEntry = { id: Math.random().toString(36).substr(2, 9), userId: this.user.id, amount: profitOrLoss, currency: 'NPR', type: profitOrLoss >= 0 ? LedgerType.TRADE_PROFIT : LedgerType.TRADE_LOSS, description: `Sold ${quantity} shares of ${this.getMarket(marketId)?.title}`, createdAt: new Date().toISOString(), status: 'COMPLETED' };
    this.ledger = [ledgerEntry, ...this.ledger];

    // Outcome Title for Trade Record
    const outcomeTitle = outcomeId && market?.outcomes ? market.outcomes.find(o => o.id === outcomeId)?.name : undefined;

    const trade: Trade = { id: Math.random().toString(36).substr(2, 9), userId: this.user.id, marketId, marketTitle: this.getMarket(marketId)?.title || 'Unknown', side, price, shares: quantity, amount: revenue, potentialWin: 0, status: profitOrLoss >= 0 ? 'WON' : 'LOST', type: 'SELL', createdAt: new Date().toISOString(), outcomeId, outcomeTitle };
    this.trades = [trade, ...this.trades];
    const remaining = position.quantity - quantity;
    if (remaining === 0) { this.positions.splice(posIdx, 1); } else { this.positions[posIdx] = { ...position, quantity: remaining }; }

    // Update Probabilities on Sell (Opposite of Buy)
    const mktIdx = this.markets.findIndex(m => m.id === marketId);
    if (mktIdx > -1) {
      const m = this.markets[mktIdx];
      let newProb = m.probability;
      ...
      if (outcomeId && outcomes) {
        // Selling YES -> Decreases Prob (Same as buying NO)
        // Selling NO -> Increases Prob (Same as buying YES)
        // Note: Reducing exposure to YES suggests market belief decreases.
        const change = side === Side.YES ? -pressure : pressure;
        outcomes = this.recalculateProbabilities(outcomes, outcomeId, change);
      } else {
        // Standard Binary
        newProb = side === Side.YES ? Math.max(1, m.probability - pressure) : Math.min(99, m.probability + pressure);
      }

      this.markets[mktIdx] = { ...this.markets[mktIdx], volume: this.markets[mktIdx].volume + revenue, probability: newProb, outcomes };
    }
