...
export const Signup: React.FC = () => {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Ensure the user is logged in immediately after sign up
            ...
            <div className="w-full max-w-md relative z-10">
                ...
                <div className="glass-panel p-8 rounded-[2rem] border border-slate-200/50 dark:border-white/5 shadow-2xl backdrop-blur-xl bg-white/50 dark:bg-slate-900/50">
                    <form onSubmit={handleSignup} className="space-y-5">
                        ...
                        <div className="pt-2">
                            <Button
                                className="w-full h-14 rounded-2xl text-sm font-black uppercase tracking-widest shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30 active:scale-[0.98] transition-all bg-indigo-600 hover:bg-indigo-700"
                                disabled={isLoading}
                                type="submit"
                            >
                                {isLoading ? <Loader2 className="animate-spin" /> : <span className="flex items-center gap-2">Create Account <ArrowRight size={16} /></span>}
                            </Button>
                        </div>
                    </form>
                </div>
