...
export const FeaturedMarketCard: React.FC<FeaturedMarketCardProps> = ({
  ...
  return (
    <div
      className={`rounded-2xl border p-3 md:p-5 relative group flex flex-col h-full
                ${isResolved
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center justify-between mb-3 md:mb-4">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-amber-400/20 flex items-center justify-center text-amber-500">
                <CategoryIcon className="w-3.5 h-3.5 md:w-4 md:h-4" />
              </div>
              <span className="text-[9px] md:text-[10px] font-black text-slate-400 tracking-widest uppercase">{market.category}</span>
              {isResolved && (
                <span className="ml-1 md:ml-2 flex items-center gap-1 text-[#9AA0A6] text-[9px] md:text-[10px] font-medium border border-[#9AA0A6]/30 px-1.5 md:px-2 py-0.5 rounded-full bg-[#9AA0A6]/5">
            ...
            {totalFeatured > 1 && (
              <div className="flex items-center gap-1 md:gap-2">
                <button
                  className="w-6 h-6 md:w-7 md:h-7 rounded-full border border-slate-700 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-colors"
                  onClick={(e) => { e.stopPropagation(); onPrev?.(); }}
                >
                  <ChevronLeft className="w-3 h-3 md:w-3.5 md:h-3.5" />
                </button>
                <span className="text-[10px] md:text-[11px] font-bold text-slate-300 w-6 md:w-8 text-center">
          ...
          <h2 className="text-lg md:text-2xl font-black text-white mb-3 md:mb-6 tracking-tight leading-tight capitalize">
            {market.title}
          </h2>

          {/* Outcomes Table Header */}
          <div className="grid grid-cols-[1fr_auto_auto] gap-2 md:gap-4 text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 md:mb-2 px-1 md:px-2">
            <div>Market</div>
            <div className="text-right w-12 md:w-14">Pays out</div>
            <div className="text-right w-12 md:w-14">Odds</div>
          </div>
          ...
          {/* Footer Stats */}
          <div className="flex items-center justify-between pt-3 md:pt-4 border-t border-dashed border-slate-800/80">
          ...
          <div className="pt-3 md:pt-4 mt-auto">
            <p className="text-[10px] md:text-xs leading-relaxed text-slate-400 truncate md:whitespace-normal">
              <span className="font-black text-white mr-1 md:mr-2">News</span>
              {market.description}
            </p>
          </div>
        </div>
